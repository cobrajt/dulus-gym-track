...
window.DulusTraining={create(api){
 const byId=new Map(api.catalog.map(e=>[e.id,e])),drafts=new Map();let active=null,saving=false,activePlan=null,saveSession=false,editingPlan=null,sessionSubstitutions={};
 ...
 const levelRank={Principiante:1,Intermedio:2,Avanzado:3},equipmentSets={gym:null,home:new Set(['Peso corporal','Sin equipamiento','Objeto doméstico','Bandas','Mancuernas','Kettlebell']),none:new Set(['Peso corporal','Sin equipamiento'])};
 const norm=s=>String(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
 function movementPattern(e){
  const text=norm((e.name||'')+' '+(e.alternativeName||'')),m=e.primaryMuscle;
  if(m==='Bíceps'&&/curl/.test(text))return 'biceps-curl';
  if(m==='Antebrazos'){if(/curl|muneca|wrist/.test(text))return 'wrist-curl';if(/hang|colgar|grip|agarre|farmer/.test(text))return 'grip';}
  if(m==='Isquiotibiales'&&/(curl|desliz)/.test(text))return 'leg-curl';
  if(m==='Tríceps'&&/(press frances|extension.*triceps|pushdown|fondos|dip|tricep|flexiones cerradas|close-grip)/.test(text))return 'triceps';
  if(m==='Hombros'){if(/press|pike|handstand/.test(text))return 'shoulder-press';if(/elevacion|raise|face pull|fly|rear delt|band pull|rotation|y-t/.test(text))return 'shoulder-raise';}
  if(m==='Trapecio'&&/(shrug|encog)/.test(text))return 'shrug';
  if(/(?:\bremo\b|\brow\b)/.test(text))return 'row';
  if(/(dominada|pull.?up|chin.?up|jalon|pulldown)/.test(text))return 'vertical-pull';
  if(/(sentadilla|squat|prensa|leg press|zancada|lunge|split|step-up)/.test(text))return 'knee-dominant';
  if(/(bisagra|peso muerto|deadlift|rumano|romanian|good morning|swing)/.test(text))return 'hinge';
  if(/(hip thrust|puente|bridge|patada|kickback|glute)/.test(text))return 'hip-extension';
  if(m==='Pecho'&&/(fly|apertura|squeeze|aduccion)/.test(text))return 'chest-fly';
  if(/(press|flexion|push.?up|fondos)/.test(text)&&m==='Pecho')return 'chest-press';
  if(/(gemelo|talon|pantorrilla|calf)/.test(text))return 'calf';
  if(/(plancha|plank|crunch|dead bug|sit-up|vacuum)/.test(text)&&m==='Core/Abdominales')return 'core';
  return null;
 }
 const noGymEquipment=new Set(['Peso corporal','Sin equipamiento','Objeto doméstico']);
 const homeProgressionLadders=[
  ['flexiones-inclinadas','flexiones','flexiones-pies-elevados'],
  ['pike-inclinada','pike-push-up','fedb-Handstand_Push-Ups'],
  ['fondos-banco','flexiones-diamante','fedb-Overhead_Triceps'],
  ['sentadilla-silla','sentadilla-peso-corporal','sentadilla-bulgara-silla'],
  ['bisagra-cadera-sin-peso','peso-muerto-una-pierna-sin-peso'],
  ['puente-talones','curl-femoral-deslizante','curl-femoral-deslizante-una-pierna'],
  ['puente-gluteos','puente-gluteo-una-pierna'],
  ['elevacion-talones','elevacion-talones-una-pierna'],
  ['dead-bug','plancha','hollow-hold']
 ];
 const homeDefaultByPattern={'chest-press':'flexiones','shoulder-press':'pike-push-up','triceps':'flexiones-diamante','row':'remo-toalla-auto','biceps-curl':'curl-biceps-auto','forearm-curl':'curl-muneca-auto','knee-dominant':'sentadilla-peso-corporal','hinge':'bisagra-cadera-sin-peso','leg-curl':'curl-femoral-deslizante','hip-extension':'puente-gluteos','calf':'elevacion-talones','core':'plancha','shoulder-raise':'elevacion-yt-prono','chest-fly':'apertura-pecho-isometrica','shrug':'encogimiento-trapecio-auto'};
 function homeDifficultyTargets(planned,current){const ladder=homeProgressionLadders.find(ids=>ids.includes(current?.id));if(!ladder)return {};const index=ladder.indexOf(current.id),lower=index>0?byId.get(ladder[index-1]):null,rawHigher=index>=0&&index<ladder.length-1?byId.get(ladder[index+1]):null,ceiling=levelRank[planned?.level]||2,higher=rawHigher&&(api.isSolo?.()||(levelRank[rawHigher.level]||2)<=ceiling)?rawHigher:null;return {lower,higher};}
 function noGymCandidates(current,map=new Map()){
  const rank=levelRank[current.level]||2,pattern=movementPattern(current),selectedIds=new Set(map.keys()),indexed=new Map(api.catalog.map((e,i)=>[e.id,i]));
  let pool=api.catalog.filter(e=>e.id!==current.id&&!selectedIds.has(e.id)&&noGymEquipment.has(e.equipment)&&(levelRank[e.level]||2)<=rank);
  const exact=pattern?pool.filter(e=>movementPattern(e)===pattern):[];
  pool=exact.length?exact:pool.filter(e=>e.primaryMuscle===current.primaryMuscle);
  return pool.map(e=>{const er=levelRank[e.level]||2,ep=movementPattern(e),overlap=(e.secondaryMuscles||[]).filter(m=>(current.secondaryMuscles||[]).includes(m)).length;let score=0;if(pattern&&ep===pattern)score+=60;if(homeDefaultByPattern[pattern]===e.id)score+=40;if(e.primaryMuscle===current.primaryMuscle)score+=18;if(!String(e.id).startsWith('fedb-'))score+=36;if(er===rank)score+=14;else if(er<rank)score+=12;score+=overlap*3;if(e.instructionLanguage!=='en')score+=6;return {e,score,index:indexed.get(e.id)||0};}).sort((a,b)=>b.score-a.score||a.index-b.index).slice(0,3).map(x=>x.e);
 }
 function alternativeCandidates(current,reason,map){if(reason==='nogym')return noGymCandidates(current,map);const rank=levelRank[current.level]||2,pattern=movementPattern(current),selectedIds=new Set(map.keys()),indexed=new Map(api.catalog.map((e,i)=>[e.id,i]));let pool=api.catalog.filter(e=>e.id!==current.id&&e.primaryMuscle===current.primaryMuscle&&!selectedIds.has(e.id)&&(levelRank[e.level]||2)<=rank+(reason==='pain'||reason==='technique'?0:1));const curated=pool.filter(e=>!String(e.id).startsWith('fedb-'));if(curated.length>=2||((reason==='pain'||reason==='technique')&&curated.length))pool=curated;if(reason==='equipment'){const different=pool.filter(e=>e.equipment!==current.equipment);if(different.length>=2)pool=different;}return pool.map(e=>{const er=levelRank[e.level]||2,ep=movementPattern(e),overlap=(e.secondaryMuscles||[]).filter(m=>(current.secondaryMuscles||[]).includes(m)).length;let score=0;if(pattern&&ep===pattern)score+=45;if(!String(e.id).startsWith('fedb-'))score+=28;if(er===rank)score+=12;else if(er<rank)score+=10;if(reason==='equipment'&&e.equipment!==current.equipment)score+=22;else if(reason!=='equipment'&&e.equipment===current.equipment)score+=8;score+=overlap*3;if(e.instructionLanguage!=='en')score+=5;return {e,score,index:indexed.get(e.id)||0};}).sort((a,b)=>b.score-a.score||a.index-b.index).slice(0,3).map(x=>x.e);}
 function alternativeReason(current,candidate){const samePattern=movementPattern(current)&&movementPattern(current)===movementPattern(candidate),sameMuscle=current.primaryMuscle===candidate.primaryMuscle,cr=levelRank[current.level]||2,nr=levelRank[candidate.level]||2,bits=[samePattern&&sameMuscle?'Mismo patrón y músculo':samePattern?'Mismo patrón de movimiento':'Mismo músculo principal'];if(noGymEquipment.has(candidate.equipment)&&!noGymEquipment.has(current.equipment))bits.push('sin pesas');else if(candidate.equipment!==current.equipment)bits.push('equipo distinto');if(nr<cr)bits.push('más sencillo');else if(nr===cr)bits.push('dificultad similar');return bits.join(' · ')+'.';}
 function substitutionPanel(current,model,map,pick){const details=node('details',undefined,'exercise-substitutions');details.dataset.exercise=current.id;details.hidden=!pick.input.checked;details.append(node('summary','Alternativas de ejercicio'));const reasonLabel=node('label','Motivo'),reason=node('select');for(const [value,text] of [['variation','Preferencia / variar'],['nogym','Sin gym / sin pesas'],['equipment','Equipo no disponible'],['technique','Técnica difícil'],['plateau','Estancamiento'],['pain','Molestia / dolor']]){const o=node('option',text);o.value=value;reason.append(o);}reasonLabel.append(reason);const help=node('p','Dulus mantiene el músculo principal y prioriza patrón, dificultad y equipo. Tú decides si conviene sustituir.','substitution-help'),results=node('div',undefined,'substitution-options');details.append(reasonLabel,help,results);
  function draw(){results.replaceChildren();if(reason.value==='pain')results.append(node('p','Si la molestia persiste, aumenta o cambia tu técnica, prioriza revisar recuperación o una evaluación adecuada antes de sustituir por tu cuenta.','substitution-warning'));const alternatives=alternativeCandidates(current,reason.value,map);if(!alternatives.length){results.append(node('p','No hay una alternativa suficientemente parecida en el catálogo actual.'));return;}for(const candidate of alternatives){const card=node('article',undefined,'substitution-card');card.dataset.exercise=candidate.id;card.append(node('strong',candidate.name),node('small',candidate.primaryMuscle+' · '+candidate.equipment+' · '+candidate.level),node('p',alternativeReason(current,candidate)));const replace=btn('Sustituir',()=>{const entries=[...map.entries()],index=entries.findIndex(([id])=>id===current.id);if(index<0)return;const replacement={...structuredClone(model),exerciseId:candidate.id,weightKg:0,coachNote:'Sustitución propuesta por Dulus. Ajusta la carga antes de asignar.'};delete replacement.videoUrl;entries[index]=[candidate.id,replacement];drafts.set(active,new Map(entries));$('plan-search').value='';planStatus.textContent=current.name+' sustituido por '+candidate.name+' en el borrador. Revisa carga y técnica antes de guardar.';summary();renderPicker();});card.append(replace);results.append(card);}}
  reason.onchange=draw;draw();return details;
 }
 ...
 function renderPicker(){
  ...
  const q=norm($('plan-search').value),found=api.catalog.filter(e=>norm(e.name+' '+e.primaryMuscle+' '+e.equipment).includes(q)),map=selected();
  ...
  for(const e of visible){
   ...
   const substitutions=substitutionPanel(e,model,map,pick);
   pick.input.disabled=saving;pick.input.onchange=()=>{if(pick.input.checked){if(map.size>=100){pick.input.checked=false;planStatus.textContent='Máximo 100 ejercicios por día.';return;}map.set(e.id,model);}else map.delete(e.id);inputs.forEach(i=>i.disabled=!pick.input.checked);substitutions.hidden=!pick.input.checked;row.classList.toggle('selected',pick.input.checked);summary();};
   row.classList.toggle('selected',pick.input.checked);row.append(pick.label,meta,controls,api.technique(e));plannerExtras(row,model,pick);row.append(substitutions);picker.append(row);
  }
 }
 ...
 function persistSeries(){return safeWrite(draftKey(),{version:3,series:seriesDone,substitutions:sessionSubstitutions});}
 function sessionItem(planned){const substitute=sessionSubstitutions[planned.exerciseId];if(!substitute||!byId.has(substitute))return planned;const item={...planned,exerciseId:substitute,weightKg:0};delete item.videoUrl;return item;}
 ...
 function sessionDifficultyPanel(planned,current){
  const plannedExercise=byId.get(planned.exerciseId);if(!plannedExercise||!current||!noGymEquipment.has(current.equipment))return document.createDocumentFragment();
  const {lower,higher}=homeDifficultyTargets(plannedExercise,current);if(!lower&&!higher)return document.createDocumentFragment();
  const details=node('details',undefined,'session-difficulty');details.append(node('summary','↕ Ajustar dificultad'),node('p','Cambia solo la variante de hoy. Tu rutina original y sus cargas no se modifican.'));const actions=node('div',undefined,'session-difficulty-actions');
  const apply=target=>{if(target.id===planned.exerciseId)delete sessionSubstitutions[planned.exerciseId];else sessionSubstitutions[planned.exerciseId]=target.id;seriesDone[planned.exerciseId]=[];persistSeries();sessionStatus.textContent='Dificultad ajustada a '+target.name+' solo para esta sesión. Las series de este ejercicio se reiniciaron.';renderExercise();};
  if(lower)actions.append(btn('Más fácil · '+lower.name,()=>apply(lower)));if(higher)actions.append(btn('Más difícil · '+higher.name,()=>apply(higher)));details.append(actions,node('small',api.isSolo?.()?'Puedes moverte por las variantes disponibles según tu control técnico.':'Dulus no ofrece una variante por encima del nivel prescrito por tu coach.'));return details;
 }
 function sessionNoGymPanel(planned){
  ...
  const activeId=sessionSubstitutions[planned.exerciseId],options=noGymCandidates(plannedExercise);
  if(activeId){const actual=byId.get(activeId);const status=node('p','Usando hoy: '+(actual?.name||activeId)+' · 0 kg. El historial guardará esta sustitución sin confundirla con '+(plannedExercise?.name||planned.exerciseId)+'.','session-substitution-active');const restore=btn('Volver al ejercicio original',()=>{delete sessionSubstitutions[planned.exerciseId];seriesDone[planned.exerciseId]=[];persistSeries();sessionStatus.textContent='Volviste al ejercicio original. Las series de este ejercicio se reiniciaron.';renderExercise();});details.append(status,restore);}
  if(!options.length){details.append(node('p','Dulus todavía no tiene una alternativa sin pesas suficientemente parecida para este ejercicio.'));return details;}
  const list=node('div',undefined,'session-no-gym-options');for(const candidate of options){const card=node('article',undefined,'session-no-gym-card');card.append(node('strong',candidate.name),node('small',candidate.primaryMuscle+' · '+candidate.equipment+' · '+candidate.level),node('p',alternativeReason(plannedExercise,candidate)));const use=btn(activeId===candidate.id?'Usando hoy ✓':'Usar solo hoy',()=>{sessionSubstitutions[planned.exerciseId]=candidate.id;seriesDone[planned.exerciseId]=[];persistSeries();sessionStatus.textContent=candidate.name+' activado solo para esta sesión. Las series de este ejercicio se reiniciaron.';renderExercise();});use.disabled=activeId===candidate.id||saveSession;card.append(use);list.append(card);}details.append(list);return details;
 }
 function renderExercise(){
  exerciseList.replaceChildren();if(!activePlan)return;
  const planned=activePlan.exercises[exerciseIndex],item=sessionItem(planned),exercise=byId.get(item.exerciseId),plannedExercise=byId.get(planned.exerciseId),name=exercise?.name||item.exerciseId,row=node('article',undefined,'guided-exercise'),substituted=item.exerciseId!==planned.exerciseId;
  const weight=item.weightKg===undefined?'Peso no indicado':item.weightKg===0?'Sin carga añadida':item.weightKg+' kg';
  row.append(node('p','Ejercicio '+(exerciseIndex+1)+' de '+activePlan.exercises.length+(substituted?' · alternativa de hoy':''),'guided-counter'),node('h3',name),node('p',setCount(item)+' series · '+item.reps+' repeticiones · '+weight,'guided-prescription'));if(substituted)row.append(node('p','Ejercicio planificado: '+(plannedExercise?.name||planned.exerciseId)+' · esta sustitución solo vale para la sesión de hoy.','session-substitution-label'));
  ...
  row.append(media);const coachNote=node('aside',undefined,'coach-note'),noteTitle=substituted?'Notas del ejercicio planificado':api.isSolo?.()?'Mis notas':'Notas de tu Coach';coachNote.append(node('strong',noteTitle),node('p',item.coachNote||(api.isSolo?.()?'Sin notas personales para este ejercicio.':'Tu coach todavía no ha añadido notas para este ejercicio.')));if(substituted)coachNote.append(node('small','Revisa que estas notas también apliquen a la alternativa de hoy.'));row.append(coachNote);if(technique)row.append(technique);
  ...
  const selected=new Set(seriesDone[planned.exerciseId]||[]);
  for(let i=0;i<setCount(item);i++){
   const line=node('div',undefined,'guided-series-line'),star=btn(selected.has(i)?'✓':String(i+1),()=>{
    if(saveSession)return;const was=selected.has(i);if(was)selected.delete(i);else selected.add(i);seriesDone[planned.exerciseId]=[...selected].sort((a,b)=>a-b);const saved=persistSeries();renderExercise();
  ...
  const complete=btn(selected.size===setCount(item)?'Ejercicio completado':'✓ Serie completada',()=>{exerciseList.querySelector('.series-check[aria-pressed=false]')?.click();});complete.className='complete-next-set';complete.disabled=saveSession||selected.size===setCount(item);const rec=timer.querySelector('.rest-recommended');if(rec)rec.textContent=item.restSets||item.restExercises?(api.isSolo?.()?'Tu plan recomienda: ':'Tu coach recomienda: ')+(item.restSets||120)+' s entre series · '+(item.restExercises||120)+' s entre ejercicios.':api.isSolo?.()?'Tiempo recomendado: 2 minutos. Ajustable según tu plan.':'Tiempo recomendado: 2 minutos. Ajustable según tu coach.';row.append(complete,stars,sessionNoGymPanel(planned),sessionDifficultyPanel(planned,exercise),adjust);if(api.bridge)row.append(api.bridge.exerciseView(activePlan,item,planned.exerciseId));exerciseList.append(row);renderHomeMode();updateProgress();paint();
 }
 ...
 function renderSession(){
  if(!activePlan)return;stop();restKind='setRest';duration.input.value=preferences.setRest||restDefault('setRest');remaining=validSeconds();paint();exerciseIndex=0;seriesDone={};sessionSubstitutions={};
  const draft=safeRead(draftKey()),done=new Set(Array.isArray(draft)?draft:[]),plannedIds=new Set(activePlan.exercises.map(x=>x.exerciseId));
  if(Number(draft?.version)>=3&&draft.substitutions&&typeof draft.substitutions==='object'){for(const [plannedId,actualId] of Object.entries(draft.substitutions)){if(plannedIds.has(plannedId)&&byId.has(actualId))sessionSubstitutions[plannedId]=actualId;}}
  for(const item of activePlan.exercises){
   const saved=Number(draft?.version)>=2&&Array.isArray(draft.series?.[item.exerciseId])?draft.series[item.exerciseId]:null;
