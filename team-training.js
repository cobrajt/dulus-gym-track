...
window.DulusTraining={create(api){
 ...
 function movementPattern(e){const text=norm((e.name||'')+' '+(e.alternativeName||'')),m=e.primaryMuscle;if(m==='Bíceps'||m==='Antebrazos')return /curl/.test(text)?'curl':null;if(m==='Isquiotibiales'&&/curl/.test(text))return 'leg-curl';if(m==='Tríceps'&&/(press frances|extension.*triceps|pushdown|fondos)/.test(text))return 'triceps';if(m==='Hombros')return /press/.test(text)?'shoulder-press':/(elevacion|raise|face pull)/.test(text)?'shoulder-raise':null;if(/(remo|row)/.test(text))return 'row';if(/(dominada|pull.?up|chin.?up|jalon|pulldown)/.test(text))return 'vertical-pull';if(/(sentadilla|squat|prensa|leg press|zancada|lunge|split)/.test(text))return 'knee-dominant';if(/(peso muerto|deadlift|rumano|romanian|good morning)/.test(text))return 'hinge';if(/(hip thrust|puente|bridge|patada|kickback)/.test(text))return 'hip-extension';if(/(press|flexion|push.?up|fondos)/.test(text)&&m==='Pecho')return 'chest-press';if(/(gemelo|talon|pantorrilla|calf)/.test(text))return 'calf';if(/(plancha|plank|crunch|dead bug)/.test(text))return 'core';return null;}
 const noGymEquipment=new Set(['Peso corporal','Sin equipamiento']);
 function noGymCandidates(current,map=new Map()){const rank=levelRank[current.level]||2,pattern=movementPattern(current),selectedIds=new Set(map.keys()),indexed=new Map(api.catalog.map((e,i)=>[e.id,i]));const pool=api.catalog.filter(e=>e.id!==current.id&&e.primaryMuscle===current.primaryMuscle&&!selectedIds.has(e.id)&&noGymEquipment.has(e.equipment)&&(levelRank[e.level]||2)<=rank);return pool.map(e=>{const er=levelRank[e.level]||2,ep=movementPattern(e),overlap=(e.secondaryMuscles||[]).filter(m=>(current.secondaryMuscles||[]).includes(m)).length;let score=0;if(pattern&&ep===pattern)score+=55;if(!String(e.id).startsWith('fedb-'))score+=32;if(er<=rank)score+=14;score+=overlap*3;if(e.instructionLanguage!=='en')score+=5;return {e,score,index:indexed.get(e.id)||0};}).sort((a,b)=>b.score-a.score||a.index-b.index).slice(0,3).map(x=>x.e);}
 ...
 function alternativeReason(current,candidate){const same=movementPattern(current)&&movementPattern(current)===movementPattern(candidate),cr=levelRank[current.level]||2,nr=levelRank[candidate.level]||2,bits=[same?'Mismo patrón y músculo':'Mismo músculo principal'];if(candidate.equipment!==current.equipment)bits.push('equipo distinto');if(nr<cr)bits.push('más sencillo');else if(nr===cr)bits.push('dificultad similar');return bits.join(' · ')+'.';}
