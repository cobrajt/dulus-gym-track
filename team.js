...
function openStudent(id){if(student?.id!==id){trainingUI.close();$('plan-panel').hidden=true;$('session-panel').hidden=true;}student=roster.find(s=>s.id===id);if(!student)return;$('student-panel').hidden=false;$('student-title').textContent=student.name;$('student-goals').textContent=student.goals.length?student.goals.join(' · '):'Sin objetivos definidos';$('goals-input').value=student.goals.join('\n');$('goals-form').hidden=!(coach||solo);$('new-plan').hidden=!(coach||solo);
 $('student-plans').replaceChildren();if(coach&&!solo){const relationship=el('section',undefined,'relationship-control');relationship.append(el('p','VÍNCULO COACH–ALUMNO','eyebrow'),el('p','Finalizar el vínculo retira tu acceso a este alumno. Su historial, rutinas, sesiones, medidas y objetivos no se borran; pasan a su espacio personal.'));const unlink=button('Finalizar vínculo con este alumno',()=>{});let armed=false,timer=null;unlink.className='relationship-danger';unlink.onclick=()=>{if(!armed){armed=true;unlink.textContent='Confirmar · retirar mi acceso';clearTimeout(timer);timer=setTimeout(()=>{armed=false;unlink.textContent='Finalizar vínculo con este alumno';},6000);return;}const targetId=student.id;run(async()=>{await check(db.rpc('dulus_remove_student',{p_student:targetId}));student=null;await refresh();say('Vínculo finalizado. El historial del alumno se conserva en su espacio personal.');});};relationship.append(unlink);$('student-plans').append(relationship);}const own=plans.filter(p=>p.student_id===id);
