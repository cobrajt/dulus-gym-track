...
function render({api,sessions,plans,roster,measurements=[],node}){
 ...
 const summary=node('div',undefined,'progress-overview'),muscleGrid=node('div',undefined,'muscle-score-grid'),curveCard=node('article',undefined,'progress-curve-card'),insights=node('div',undefined,'progress-insights'),exerciseDetails=node('details',undefined,'progress-exercise-details'),bodySection=node('section',undefined,'body-progress');let selectedBody='';
 exerciseDetails.append(node('summary','Ver detalle por ejercicio'));section.append(summary,node('h3','Mapa muscular de progreso'),muscleGrid,curveCard,insights,exerciseDetails,bodySection);
 ...
 function drawBody(){bodySection.replaceChildren(node('h3','Medidas corporales'),node('p','Cambios medidos en el tiempo. Dulus no interpreta subir o bajar como bueno o malo sin considerar el objetivo.','progress-note'));const rows=measurements.filter(r=>r.student_id===studentId).sort((a,b)=>String(a.measured_on).localeCompare(String(b.measured_on))),defs=[['weight','Peso','kg',r=>r.weight_kg],['waist','Cintura','cm',r=>r.values_cm?.waist],['glutes','Glúteos','cm',r=>r.values_cm?.glutes],['hips','Cadera','cm',r=>r.values_cm?.hips],['chest','Pecho','cm',r=>r.values_cm?.chest]],series=defs.map(([key,label,unit,get])=>({key,label,unit,points:rows.map(r=>({date:r.measured_on,value:Number(get(r))})).filter(p=>Number.isFinite(p.value))})).filter(x=>x.points.length);if(!series.length){bodySection.append(node('p','Aún no hay medidas corporales online para este perfil.'));return;}if(!series.some(x=>x.key===selectedBody))selectedBody=series[0].key;const controls=node('div',undefined,'progress-controls'),select=node('select');select.setAttribute('aria-label','Medida corporal para analizar');for(const x of series)select.append(new Option(x.label,x.key));select.value=selectedBody;controls.append(node('label','Medida corporal'),select);bodySection.append(controls);const chosen=series.find(x=>x.key===selectedBody),card=node('article',undefined,'progress-curve-card'),first=chosen.points[0],last=chosen.points.at(-1),width=640,height=220,pad=34,vals=chosen.points.map(p=>p.value),rawMin=Math.min(...vals),rawMax=Math.max(...vals),margin=Math.max((rawMax-rawMin)*.15,rawMax*.02,1),min=rawMin-margin,max=rawMax+margin,range=Math.max(1,max-min),svg=svgEl('svg',{viewBox:`0 0 ${width} ${height}`,role:'img','aria-label':'Curva de '+chosen.label}),y=v=>height-pad-(v-min)/range*(height-pad*2),x=i=>pad+(chosen.points.length===1?0:(width-pad*2)*i/(chosen.points.length-1));card.append(node('h3','Curva · '+chosen.label));svg.append(svgEl('polyline',{points:chosen.points.map((p,i)=>x(i)+','+y(p.value)).join(' '),class:'progress-line'}));for(let i=0;i<chosen.points.length;i++)svg.append(svgEl('circle',{cx:x(i),cy:y(chosen.points[i].value),r:5,class:'progress-dot'}));card.append(svg);const labels=node('div',undefined,'progress-curve-labels');labels.append(node('small',first.date),node('strong',round(last.value,1)+' '+chosen.unit),node('small',last.date));card.append(labels);const stats=node('div',undefined,'progress-selected-stats'),change=last.value-first.value;for(const [v,l] of [[round(last.value,1)+' '+chosen.unit,'Actual'],[(change>=0?'+':'')+round(change,1)+' '+chosen.unit,'Cambio desde inicial'],[chosen.points.length,'Registros']]){const c=node('div');c.append(node('strong',String(v)),node('small',l));stats.append(c);}card.append(stats);bodySection.append(card);select.onchange=()=>{selectedBody=select.value;drawBody();};}
 function redraw(){drawBody();const muscles=studentMuscles(studentId,data);if(!muscles.length){summary.replaceChildren();muscleGrid.replaceChildren(node('p','El progreso aparecerá al guardar ejercicios completados.'));curveCard.replaceChildren();insights.replaceChildren();exerciseDetails.replaceChildren(node('summary','Ver detalle por ejercicio'));return;}
