...
async function refresh(){
 ...
 if(team.owner_id===user.id){const button=document.createElement('button');button.type='button';button.className='secondary-action';button.textContent='Crear invitación para un alumno';button.onclick=()=>run(card,async()=>{const code=await rpc('dulus_create_invitation',{p_team:team.id});$('invitation-result').textContent='Código: '+code;say('Invitación creada. Puedes copiar el código y compartirlo con tu alumno.');});card.append(button);}
